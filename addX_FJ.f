...
!!!!!!!!!!!!!!!!!!!!initialization call to user subroutine!!!!!!!!!!!!!!
        INIT = 0
      call XCLONO2 (WW,TT,XP,XM, X,INIT,TITLNEW)
...
!!!!!!!!!!!!!!!!!!!primary high-resolution wavelength loop!!!!!!!!!!!!!!
       do J = K1,K2
        ...
        call XCLONO2 (W(J), TT,XP,XM, XNEW, INIT, TITLNEW)
...
!-----------------------------------------------------------------------
      subroutine XCLONO2 (WW,TT,PP,MM, XXWT,INIT, TITLNEW)
!-----------------------------------------------------------------------
!   WW = wavelength (nm)
...
!     if INIT.eq.0 .then reads in any tables and sets Xsect name to TITLNEW
      ...
      if(INIT .eq. 0) then
        ...
        TITLNEW = 'CLONO2'
        FTBL = 'XCLONO2_230K_IUPACtbl.dat'
...
!---interpolate X-section vs. Wavelength, T=230
        IW = 1
        do I = 2,NW-1
          if (WW .gt. W(I)) IW = I
        enddo
